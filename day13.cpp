...
struct Packet {
    ...
    static Packet parse(const string& packet_text) {
        deque<int> tokens;
        string current_number;
        for (char ch : packet_text) {
            if (ch >= '0' && ch <= '9') {
                current_number += ch;
    ...
    static bool is_smaller(const Packet& packet1, const Packet& packet2) {  // return true if packet1 is smaller
        ...
        while (!tokens1.empty() && !tokens2.empty()) {
            ...
            if (t1 == Packet::LIST_START) {            // "package up" packet2's integer in a list
                tokens2.insert(tokens2.begin(), { t2, Packet::LIST_END });
            } else if (t2 == Packet::LIST_START) {     // "package up" packet1's integer in a list
                tokens1.insert(tokens1.begin(), { t1, Packet::LIST_END });
            } else                                     // at this point, both tokens must be different integers
                return t1 < t2;
        }
...
int day13a(const vector<string>& lines) {
    ...
    for (int row = 0; row < lines.size(); row += 3) {
        packet_pairs.emplace_back(Packet::parse(lines[row]), Packet::parse(lines[row+1]));
    }
    int result = 0;
    for (int i = 0; i < packet_pairs.size(); i++) {
        result += Packet::is_smaller(packet_pairs[i].first, packet_pairs[i].second) ? i + 1 : 0;
    }
    return result;
}
...
int day13b(const vector<string>& lines) {
    ...
    for (int row = 0; row < lines.size(); row += 3) {
        packets.insert(packets.end(), { Packet::parse(lines[row]), Packet::parse(lines[row+1]) });
    }
    sort(packets.begin(), packets.end(), Packet::is_smaller);
    int result = 1;
